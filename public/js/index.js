...
window.entrarSala = async function(codigo) {
    const resposta = await fetch('/api/sala/entrar', {
        method: 'POST',
    ...
    if (resposta.ok) {
        window.location.href = `/multiPlayer/${dados.codigo}`;
    } else {
        alert(dados.erro);
    }
}
...
window.criarSala = async function() {
    const pontuacaoMaxima = document.getElementById('maxPoints').value;
    const privada = document.getElementById('roomPrivacy').checked;
    const codigo = document.getElementById('roomCodeConfig').value;

    const resposta = await fetch('/api/sala/criar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pontuacaoMaxima, privada, codigo })
    });
...
window.entrarComCodigo = async function() {
    const codigo = document.getElementById('roomCodeInput').value.toUpperCase();
